...
auto_arima = pm.auto_arima(train, seasonal=True, m=7, stepwise=True, suppress_warnings=True, max_p=10, max_q=10)
...
arima_model_train = ARIMA(train, order=auto_arima.order, seasonal_order=auto_arima.seasonal_order)
...
mse_test_arima = mean_squared_error(test, arima_test_predictions)
mae_test_arima = mean_absolute_error(test, arima_test_predictions)

print('ARIMA Test MSE: %.3f' % mse_test_arima)
...
model.fit(new_train_X, new_train_Y, epochs=100, batch_size=64, validation_split=0.05)
...
print('ARIMA Test MSE: %.10f' % mse_test_arima)
...
test = test[10:]
pred_final = pred_final[10:]

# Calculate total mse and mae
...
test_dates = df.index[split_index + 10:]
test_resid = test_resid[10:]
ann_predictions_rescaled = ann_predictions_rescaled[10:]
